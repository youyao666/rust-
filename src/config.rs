    ...
    fn validate(&self) -> Result<()> {
        ...
        if self.bind_addr.parse::<SocketAddr>().is_err() {
            return Err(TrojanError::Config(format!(
                "Invalid bind_addr socket address: {}",
                self.bind_addr
            )));
        }
        ...
        validate_host_port("fallback_addr", &self.fallback_addr)?;
...
fn validate_host_port(field: &str, value: &str) -> Result<()> {
    ...
    let (host, port) = value.rsplit_once(':').ok_or_else(|| {
        TrojanError::Config(format!(
            "Invalid {}: expected host:port or socket address, got {}",
            field, value
        ))
    })?;
    ...
    let _port = port.parse::<u16>().map_err(|_| {
        TrojanError::Config(format!("Invalid {}: port is invalid ({})", field, value))
    })?;
...
mod tests {
    ...
    #[test]
    fn test_validate_host_port_accepts_socket_addr() {
        assert!(validate_host_port("fallback_addr", "127.0.0.1:80").is_ok());
        assert!(validate_host_port("fallback_addr", "[::1]:443").is_ok());
    }

    #[test]
    fn test_validate_host_port_accepts_hostname_port() {
        assert!(validate_host_port("fallback_addr", "example.com:443").is_ok());
        assert!(validate_host_port("fallback_addr", "localhost:8080").is_ok());
    }

    #[test]
    fn test_validate_host_port_rejects_invalid_values() {
        assert!(validate_host_port("fallback_addr", "example.com").is_err());
        assert!(validate_host_port("fallback_addr", "example.com:abc").is_err());
        assert!(validate_host_port("fallback_addr", ":443").is_err());
    }
}
