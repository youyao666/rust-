...
{
    // ── 步骤 1：Peek 首批数据 ─────────────────────────────────────────────
    let peeked = peek_client_data(&mut stream, PEEK_SIZE).await?;
    ...
    // ── 步骤 2：非消耗式协议/密码校验 ─────────────────────────────────────
    let parse_result = Header::try_parse_from_buf(&peeked);
    ...
    let header = match parse_result {
        ...
        // 数据不足，无法完整解析 header → fallback
        Ok(None) => {
            warn!(
                "[{}] Incomplete header ({} bytes) → fallback to {}",
                peer_addr,
                peeked.len(),
